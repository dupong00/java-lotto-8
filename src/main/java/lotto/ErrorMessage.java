package lotto;

public enum ErrorMessage {
    INVALID_LOTTO_NUMBER_COUNT("로또 번호는 6개여야 합니다."),
    DUPLICATE_LOTTO_NUMBER("당첨 번호는 중복될 수 없습니다."),
    INVALID_LOTTO_NUMBER_BLANK("당첨 번호에 공백 입력이 불가합니다."),
    INVALID_LOTTO_NUMBER("당첨 번호는 숫자만 입력 가능합니다."),
    INVALID_LOTTO_NUMBER_RANGE("로또 번호는 1부터 45 사이의 숫자여야 합니다."),


    INVALID_MONEY_NUMBER("금액은 숫자만 입력가능합니다."),
    INVALID_MONEY_MIN_ORDER("구입 금액은 1000원 이상이어야 합니다."),
    INVALID_MONEY_MIN_UNIT("구입 금액은 1000원 단위에 맞춰 구매 가능합니다"),

    INVALID_BONUS_NUMBER("보너스 번호는 숫자만 입력 가능합니다."),
    DUPLICATE_BONUS_NUMBER("보너스 번호는 당첨 번호와 중복될 수 없습니다.")
    ;

    private static final String PREFIX = "[ERROR] ";
    private final String message;

    ErrorMessage(String message) {
        this.message = message;
    }

    public String getMessage() {
        return PREFIX + message;
    }
}
